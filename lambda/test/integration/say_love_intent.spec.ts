...
import * as r from '../fixtures/requests/say_love_intent.json'; // tslint:disable-line

const request:RequestEnvelope = <RequestEnvelope>r;
const assert = new Assertion();
let skill_response:ResponseEnvelope;

describe('Pammie : SayLoveIntent', function () {
  beforeEach(() => {
    this.timeout(5000);
  ...
  it('it responds with the expected output speech', () => {
    assert.correctOutputSpeechIncludesTextMatch(skill_response, /(Hippo|Matt Matt) loves you (more than anything Llama|so flipping much|more than you can ever imagine)/g);
  });
